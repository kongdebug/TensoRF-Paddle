...
def eval_sh(deg, sh, dirs):
    ...
    Works with paddle/np/jnp.
    ... Can be 0 or more batch dimensions.
    :param deg: int SH max degree. Currently, 0-4 supported
    :param sh: paddle.Tensor SH coeffs (..., C, (max degree + 1) ** 2)
    :param dirs: paddle.Tensor unit directions (..., 3)
    :return: (..., C)
    """
...
def eval_sh_bases(deg, dirs):
    ...
    :param deg: int SH max degree. Currently, 0-4 supported
    :param dirs: paddle.Tensor (..., 3) unit directions
    :return: paddle.Tensor (..., (deg+1) ** 2)
    """
    assert deg <= 4 and deg >= 0
    result = paddle.empty((*dirs.shape[:-1], (deg + 1) ** 2), dtype=dirs.dtype)
    result[..., 0] = C0
    if deg > 0:
        x, y, z = dirs.unbind(-1)
        result[..., 1] = -C1 * y
