...
def OctreeRender_trilinear_fast(rays, tensorf, chunk=4096, N_samples=-1, ndc_ray=False, white_bg=True, is_train=False):
    ...
    return paddle.concat(rgbs), None, paddle.concat(depth_maps), None, None

@paddle.no_grad()
def evaluation(test_dataset, tensorf, args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
               white_bg=False, ndc_ray=False, compute_extra_metrics=True):
    ...
    for idx, samples in tqdm(enumerate(test_dataset.all_rays[0::img_eval_interval]), file=sys.stdout):
        ...
        rays = samples.reshape([-1,samples.shape[-1]])

        rgb_map, _, depth_map, _, _ = renderer(rays, tensorf, chunk=4096, N_samples=N_samples,
                                        ndc_ray=ndc_ray, white_bg = white_bg)
        rgb_map = paddle.clip(rgb_map, 0.0, 1.0)

        rgb_map, depth_map = rgb_map.reshape([H, W, 3]).cpu(), depth_map.reshape([H, W]).cpu()

        depth_map, _ = visualize_depth_numpy(depth_map.numpy(),near_far)
        if len(test_dataset.all_rgbs):
            gt_rgb = test_dataset.all_rgbs[idxs[idx]].reshape([H, W, 3])
            loss = paddle.mean((rgb_map - gt_rgb) ** 2)
...
def evaluation_path(test_dataset,tensorf, c2ws, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    for idx, c2w in tqdm(enumerate(c2ws)):
        ...
        c2w = paddle.to_tensor(c2w).astype('float32')
        rays_o, rays_d = get_rays(test_dataset.directions, c2w)  # both (h*w, 3)
        if ndc_ray:
            rays_o, rays_d = ndc_rays_blender(H, W, test_dataset.focal[0], 1.0, rays_o, rays_d)
        rays = paddle.concat([rays_o, rays_d], 1)  # (h*w, 6)
